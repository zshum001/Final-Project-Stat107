...
4.  **Nitrogen Oxide** ($NOx$)**:** Similar to $NO_2$, a gas that is produced by combustion of fossil fuels, contributes to acid rain, smog, and ground level ozone.
...
```{r}
# ANOVA test ~ Concentrations of Pollutants
fit <- lm(Concentration ~ Pollutant, data = airM_2)
anova(fit)

# Since the p-value is less than 0.05, we have evidence that the type of pollutant does have an effect on its  concentration. We can reject the null hypothesis that the type of pollutant does not have an effect on concentration.
```
...
summary(model1)
```
There seems to be an exponential relationship between these two variables. 
...
In an attempt to track the inverse relationship that was previously observed in the line graph between carbon monoxide and nitrogen oxide, a regression analysis was conducted. After plotting the data points, there seemed to be an exponential relationship between the two variables and a regression analysis produced a r squared value of 0.69. Therefore, there seems to be a moderate correlation between the variables. When carbon monoxide concentration levels go up, nitrogen oxide levels may be predicted to decrease at a decreasing rate, following a slow exponential decay. 
